...
"""
Główny moduł aplikacji. Odpowiada za:
  - Twilio webhook (/twilio/incoming) — generowanie TwiML z adresem WebSocket
  - WebSocket endpoint (/ws) — uruchomienie pipeline'u Pipecat dla każdego połączenia
  - Inicjalizację serwisów: STT (Deepgram), LLM (OpenAI/Groq), TTS, FlowManager
  - Zapis logu rozmowy po zakończeniu połączenia

Architektura: Twilio → WebSocket → Pipecat → Deepgram STT → LLM → FlowManager → TTS → Twilio
"""
import time
import os
import sys
...
from services.tts_factory import create_tts_service, DEFAULT_ELEVENLABS_VOICE_ID
...
async def run_call_pipeline(
    ...
    # 🧪 TEST: wymuszony Groq gpt-oss-120b na tenancie testowym, po ID — NIE po wartości
    # llm_provider (w bazie ten tenant ma "openai", więc bez tego override'u zawsze wróci
    # do GPT-4.1-mini niezależnie od tego co ustawimy w gałęzi "groq" niżej). Panel nie ma
    # jeszcze pola do wyboru dostawcy LLM, więc to jedyny sposób testowania na tym tenancie
    # bez ręcznej zmiany w bazie. Usuń ten blok gdy skończysz porównywać z GPT-4.1-mini.
