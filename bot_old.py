...
async def incoming(request: Request):
    ...
    if tenant.get("is_blocked"):
        logger.warning(f"🚫 Tenant {tenant['id']} BLOCKED - rejecting call")
        return Response(
            content='<?xml version="1.0"?><Response><Say language="pl-PL">Przepraszamy, linia jest chwilowo niedostępna. Prosimy spróbować później.</Say><Hangup/></Response>',
            media_type="application/xml"
        )
    ...
    try:
        while True:
            ...
            if event == "start":
                ...
                # Przywitanie - użyj first_message z bazy lub domyślne
                tenant_name = conv.tenant.get('name') or conv.tenant.get('business_name', 'salon')
                greeting = conv.tenant.get('first_message') or f"Dzień dobry, tu {tenant_name}. W czym mogę pomóc?"
